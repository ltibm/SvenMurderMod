#include "weapon_csknifem_projtile"
enum KnifeAnimation
{
	KNIFE_IDLE = 0,
	KNIFE_SLASH1,
...
int weapon_csknife_pos = 8;
class weapon_csknifem : ScriptBasePlayerWeaponEntity
{
	...
	void Spawn()
	{
		...
		self.m_iDefaultAmmo = cvar_KnifeDefaultGive.GetInt();
		self.FallInit();
	}
	...
	string GetVModel()
	{
		if(cvar_UseTRModel.GetInt() > 0)
		{
			return "models/csknife/v_knife_tr.mdl";
		}
		return "models/cs16/csknife/v_knife.mdl";
	}
	void Precache()
	{
		...
		g_Game.PrecacheGeneric( "sprites/" + "cs16/weapon_csknifem.txt" );
	}
	...
	bool GetItemInfo( ItemInfo& out info )
	{
		...
		{
			info.iMaxAmmo1		= cvar_KnifeMaxAmmo.GetInt();
		}
	...
	bool AddToPlayer( CBasePlayer@ pPlayer )
	{
		if( BaseClass.AddToPlayer ( pPlayer ) )
		{
			...
			NetworkMessage csknife( MSG_ONE_UNRELIABLE, NetworkMessages::WeapPickup, pPlayer.edict() );
				csknife.WriteLong( g_ItemRegistry.GetIdForName("weapon_csknifem") );
			csknife.End();
	...
	void ModelThink()
	{
		...
		g_EntityFuncs.SetModel( self, self.GetW_Model( "models/cs16/csknife/w_knife.mdl") );
		m_pPlayer.pev.viewmodel = this.GetVModel();
		SetThink(null);
...
class AmmoKnife : ScriptBasePlayerAmmoEntity
{
	void Spawn()
	{
		Precache();
...
{
	return "weapon_csknifem";
}

void RegisterCSKNIFEM()
{
	g_CustomEntityFuncs.RegisterCustomEntity( GetCSKNIFEName(), GetCSKNIFEName() );
	g_CustomEntityFuncs.RegisterCustomEntity( "AmmoKnife", "knife" );
	g_ItemRegistry.RegisterWeapon( GetCSKNIFEName(), "cs16", "knife" );
